...
def addmusic():
    if request.method == 'POST':
        ...
        # with sqlite3.connect("music.db") as users:
        #     cursor = users.cursor()
        #     cursor.execute("INSERT INTO MUSIC \
		# 	(song_name, artist_name, genre, year_released, country_released, image_cover_link, youtube_link, styles) VALUES (?,?,?,?,?,?,?,?)",
        #                    (song_name, artist_name, genre, year_released, country_released, image_cover_link, youtube_link, styles))
        #     users.commit()
        return render_template("index.html")
    else:
...
def allmusic():
    ...
    # cursor.execute('SELECT * FROM MUSIC WHERE genre LIKE ?',
    #                ('%' + query + '%',))
